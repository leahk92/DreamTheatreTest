...
const monthTab = document.querySelectorAll(".month-tabs [role='tab']"); //배열

// 탭 패널
const panelBox = document.querySelectorAll(".tabContents .panelBox"); //배열
const pdPanel = document.querySelectorAll(".panelBox .pdContent");
const calPanel = document.querySelectorAll(".panelBox .calContent");

const tabNext = document.querySelector("#tab-next");
const tabPrev = document.querySelector("#tab-prev");

const SELECT_CLASSNAME = "select";

let currentMode = "pd"; // 기본 모드는 pd (프로덕션)

// 활성화 탭 색상 표시
function selectTab(selectedMonth) {
   selectedMonth.classList.add(SELECT_CLASSNAME);
}
...
function initSet() {
   ...
   // 모든 패널을 숨김 처리
   panelBox.forEach((panel) => (panel.hidden = true));
...
function switchTab(newTab, mode) {
   ...
   // 탭 색상 변경
   selectTab(newTab);
   activeTab.classList.remove(SELECT_CLASSNAME);
...
calTab.addEventListener("click", () => {
   ...
   calTab.style.opacity = "1";
   pdTab.style.opacity = "0.6";
...
function switchArrTab(newIndex) {
   ...
   newTab.setAttribute("aria-selected", "true");
   newPanel.hidden = false;
   newTab.classList.add(SELECT_CLASSNAME);
